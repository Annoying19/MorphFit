...
# ✅ Download model lazily (not at startup)
def ensure_model_exists():
    model_path = os.path.join("app", "siamese_model.pt")
    model_url = "https://drive.google.com/uc?export=download&id=1KoyusogBnMQEtqAaY2JvlbaV1vRbHMql"
    if not os.path.exists(model_path):
        print("🔽 Downloading Siamese model...")
        try:
...
def memory_usage():
    ...
    return jsonify({"memory_usage_MB": round(mem_mb, 2)})

# ✅ Call this route to ensure model is available
@app.route("/prepare-model", methods=["GET"])
def prepare_model():
    ensure_model_exists()
    return jsonify({"message": "Model checked/downloaded."})
    ...
    try:
        with open(file_path, 'rb') as f:
            ...
            if len(image_bytes) < 1000:
                return abort(400, description="Corrupted image")
            img = Image.open(io.BytesIO(image_bytes))
            img.verify()
        return send_from_directory(upload_folder, filename)
    except Exception as e:
        return abort(500, description=f"Unreadable image: {e}")
